# learn-JavaScript

[0-1 强类型与弱类型、静态类型与动态类型](0-1%20强类型与弱类型、静态类型与动态类型.md)
[[0-1 强类型与弱类型、静态类型与动态类型]]

## 词法、语法
### 编译时词法

[1 词法文法](1%20词法文法.md)

### 运行时语法

[值类型](值类型.md)

[2-3 表达式与操作符](2-3%20表达式与操作符.md)

[类型转换](类型转换.md)

[声明](声明.md)

[语句](语句.md)

[对象](对象.md)

[函数](函数.md)

[模块](JavaScript/运行时语法/模块.md)

[严格模式](JavaScript/运行时语法/严格模式.md)

## 运行时机制与特性


## 内置对象

[String](String.md)

[Number](Number.md)

[BigInt](BigInt.md)

[Boolean](Boolean.md)

[Symbol](Symbol.md)

[Object](Object.md)

[Function](Function.md)

[Array](Array.md)

## 面向对象编程

[异步编程解决方案](./doc/异步编程解决方案.md)

## 手写实现

[Promise 实现](Promise实现.md)

[柯里化](柯里化.md)

## DOM

[DOM](Document.md)


## 宿主环境

[浏览器工作原理](浏览器工作原理.md)

[JavaScript 引擎基本原理_V8](JavaScript引擎基本原理_V8.md)

[JavaScript 运行时环境](JavaScript运行时环境.md)

[事件循环](事件循环.md)

[内存管理与垃圾回收](内存管理与垃圾回收.md)

[执行上下文](执行上下文.md)

[call stack](callStack.md)

[作用域与作用域链]()

[闭包](闭包.md)

[对象之快慢属性](对象之快慢属性与隐藏类.md)


[数组](数组.md)

[script 之 defer、async、module](script之defer、async、module.md)


[对象之内联缓存](对象之内联缓存.md)
## 性能优化

[](性能优化.md)

[Performance 面板](Performance面板.md)
## 函数式编程

[函数式编程](函数式编程.md)



## 工具

[compiler explorer](https://godbolt.org/)

[freecodecamp](https://www.freecodecamp.org/)

[courses.wesbos.com](https://courses.wesbos.com/account)

[codepen](https://codepen.io/challenges)

[codewars](https://www.codewars.com/kata/latest/my-languages?beta=false)

[edabit.com](https://edabit.com/challenges)

[exercism](https://exercism.org/dashboard)

[codingame](https://www.codingame.com/ide/puzzle/onboarding)

[jskatas](https://jskatas.org/)

[scrimba](https://scrimba.com/dashboard#overview)

[cssbattle](https://cssbattle.dev/learn)

## 参考

[ECMAScript 5.1-github](https://es5.github.io/#x8.7es5)

[ECMA-262](https://www.ecma-international.org/wp-content/uploads/ECMA-262.pdf)

[ES5-中文](https://www.ecma-international.org/wp-content/uploads/ECMA-262.pdf)

[现代 JavaScript 教程](https://zh.javascript.info/)

[JavaScript GuideBook](https://tsejx.github.io/javascript-guidebook/browser-object-model/browser-working-principle/construction-of-render-tree/)

[【译】JavaScript 工作原理：V8 引擎中5个优化代码的技巧](https://lyn-ho.github.io/posts/4d26265b/)

[代码整洁的 JavaScript-rldb](https://www.yuque.com/lpldplws/web/uc3snh?#)

[前端设计能力提升-gvfn](https://www.yuque.com/lpldplws/web/ugszzbc7q8hrpx4o?#)

[web.dev](https://web.dev)

[使用 JavaScript 来实现解释器和编译器系列教程](https://hsiaosiyuan0.gitbook.io/icj)